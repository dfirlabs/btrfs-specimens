...
#   an array containing the arguments for mkfs.btrfs
#
create_test_image_file()
{
	...
	mkfs.btrfs -q ${ARGUMENTS[@]} ${IMAGE_FILE};
}
...
SPECIMENS_PATH="specimens/mkfs.btrfs";
...
IMAGE_SIZE=114294784;
SECTOR_SIZE=512;

# Create an btrfs file system without a journal
create_test_image_file_with_file_entries "${SPECIMENS_PATH}/ext2.raw" ${IMAGE_SIZE} ${SECTOR_SIZE} "-L btrfs_test";
